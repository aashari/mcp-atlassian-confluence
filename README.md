...
- **Spaces Management**:
  - `list-spaces`: List all available Confluence spaces with their IDs, keys, types, and URLs
  - `get-space`: Get detailed information about a specific Confluence space by ID

### IP Address Lookup (from boilerplate)

- **IP Information**:
  - `get-ip-details`: Get details about a specific IP address or the current device
...
- **`src/controllers/`**: Business logic for operations.
  ...
  - `ipaddress.controller.ts`: Handles IP address lookup operations
...
npm start -- get-ip-details [ip-address]
```
